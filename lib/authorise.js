...
function getBearerToken (done) {
  var headerToken = this.req.headers.authorization,
  ...
  if (methodsUsed > 1) {
    return done(new error.BadMethodError('Only one method may be used to authenticate at a time (Auth header, GET or POST).'));
  } else if (methodsUsed === 0) {
    return done(new error.InvalidCredentialsError('The access token was not found'));
  }
  ...
  if (headerToken) {
    ...
    if (!matches) {
      return done(new error.InvalidHeaderError('Malformed auth header'));
    }
  ...
  if (postToken) {
    if (this.req.method === 'GET') {
      return done(new error.BadMethodError('Method cannot be GET When putting the token in the body.'));
    }

    if (!this.req.is('application/x-www-form-urlencoded')) {
      return done(new error.BadMethodError('When putting the token in the ' +
...
function checkToken (done) {
  ...
  this.model.getAccessToken(this.bearerToken, function (err, token) {
    if (err) return done(new error.InternalError(err));

    if (!token) {
      return done(new error.InvalidCredentialsError('The access token provided is invalid.'));
    }

    if (token.expires !== null &&
      (!token.expires || token.expires < new Date())) {
      return done(new error.InvalidCredentialsError('The access token provided has expired.'));
    }
